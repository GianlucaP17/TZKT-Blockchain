//
//  BlockCountRemoteDataSource.swift
...
public class BlockCountRemoteDataSource: BlockCountRemoteDataSourceInterface {
    ...
    public func getBlockCount(level: Int) async throws -> Int {
        let networkService = URLSessionService()
        let dataDecoder = JSONDecoderService()
        let postManager = NetworkManager<Int>(networkService: networkService, dataDecoder: dataDecoder)
        
        guard let url = URL(string: urlString + "\(level)") else {
